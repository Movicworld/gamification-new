        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                ...
                <div class="block-options">
                    <a href="{{ route('user.referrals.export', $user->id) }}" class="btn btn-sm btn-primary">
                            ...
                            <tbody>
                                @foreach ($ref as $inf)
                                    <tr>
                                        ...
                                        <td>
                                            @if($inf?->wallet?->base_currency === 'NGN')
                                                &#8358;{{ number_format($inf->wallet->balance, 2) }}
                                            @elseif($inf?->wallet?->base_currency === 'USD')
                                                ${{ number_format($inf->wallet->usd_balance, 2) }}
                                            @else
                                                {{ $inf?->wallet?->base_currency }}
                                                {{ number_format($inf?->wallet?->bonus ?? 0, 2) }}
                                            @endif
                                        </td>
                                        <td>
